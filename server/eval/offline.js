/**
 * OFFLINE evaluation harness — plan.md §8.
 *
 * The companion to `run.js`. Two different jobs, deliberately kept apart:
 *
 *   run.js      scores the REAL seeded MongoDB after a seed run. Measures the
 *               system as deployed. Needs MONGODB_URI. This is the primary
 *               harness and the source of eval/results.md.
 *
 *   offline.js  runs its own labelled set through the pipeline live, with
 *               in-memory storage and NO DATABASE. Use it to check the Gemma
 *               engine in isolation — after a prompt change, on a laptop with
 *               no Atlas access, or in CI.
 *
 *   node eval/offline.js            full run (~15 min, ~40 model calls)
 *   node eval/offline.js --triage   triage metrics only, skip dedupe
 *
 * Writes eval/offline-results.md — deliberately NOT `RESULTS.md`, which would
 * collide with `results.md` on case-insensitive filesystems (NTFS, APFS) and
 * produce a file that cannot be checked out cleanly.
 *
 * Publish the numbers INCLUDING the bad ones — the point is credibility, and a
 * table with an ugly cell beats a round claim with nothing behind it.
 */
...
fs.writeFileSync(path.join(evalDir, 'offline-results.md'), md);
console.log(`\nWrote ${path.join(evalDir, 'offline-results.md')}`);
